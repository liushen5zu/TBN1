...
function addFav(t) {document.write('<a href="'+window.location.href+'" title="'+document.title.replace(/<|>|'|"|&/g, '')+'" rel="nofollow" onclick="window.external.addFavorite(this.href, this.title);return false;">'+t+'</a>');}
function Album(id, s,itemid,moduleid) {
	for(var i=0; i<3; i++) {Dd('t_'+i).className = i==id ? 'ab_on' : 'ab_im';}
	Dd('abm').innerHTML = '<img src="'+s+'" onload="if(this.width>240){this.width=240;}" onclick="PAlbum(this,'+itemid+','+moduleid+');" onmouseover="SAlbum(this.src);" onmouseout="HAlbum();" id="DIMG"/>';
}
function SAlbum(s) {
...
function PAlbum(o,id,mid) {if(o.src.indexOf('nopic240.gif')==-1) View(o.src,id,mid);}
function HAlbum() {Dh('imgshow');}
function Dsearch() {
	if(Dd('kw').value.length < 1 || Dd('kw').value == L['keyword_message']) {
		Dd('kw').value = '';
		window.setTimeout(function(){Dd('kw').value = L['keyword_message'];}, 500);
		return false;
	}
	return true;
}
...
function View(s,id,mid) {window.open(EXPath+'view.php?img='+s+'&id='+id+'&mid='+mid);}
...
function setModule_news(i, n) {Dd('destoon_search').action = DTPath+'api/search.php';Dd('destoon_moduleid').value = i;searchid = i;Dd('destoon_select').value = n;$('#search_module').fadeOut('fast');Dd('destoon_kw').focus();}
...
function img_tip_b(o, i) {
	if(i) {
		if(i.indexOf('nopic.gif') == -1) {
			...
			var X = o.offsetLeft + leftpos + 215;
			var Y = o.offsetTop + toppos;
			Dd('img_tip').style.left = X + 'px';
			Dd('img_tip').style.top = Y + 'px';
			Ds('img_tip');
			Inner('img_tip', '<img src="'+s+'" onload="if(this.width<210) {Dh(\'img_tip\');}else if(this.width>300){this.width=300;}Dd(\'img_tip\').style.width=this.width+\'px\';"/>')
		}
	} else {
		Dh('img_tip');
	}
